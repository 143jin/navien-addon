...
def get_power_state(v, idx, preset_bits):
    ...
    if preset_bits.get('hotwater', 0) == 1:
        return 'hotwater'
    if preset_bits.get('away', 0) == 1:
        return 'away'
    
    # 프리셋 모드 없으면 heating/ off 판단
    bit = format(int(v, 16), '05b')[idx]
...
for message_flag in ['81', 'C5', 'C7']:
    ...
    팬트리난방.register_status(message_flag = message_flag, attr_name = 'preset_mode', topic_class = 'preset_mode_state_topic', regex = r'00[\da-fA-F]{2}([\da-fA-F]{2})', process_func = lambda v: 'away' if format(int(v, 16), '05b')[0] == '1' else 'off')
    ...
    팬트리난방.register_status(message_flag = message_flag, attr_name = 'preset_mode', topic_class = 'preset_mode_state_topic', regex = r'00[\da-fA-F]{6}([\da-fA-F]{2})', process_func = lambda v: 'hotwater' if format(int(v, 16), '05b')[0] == '1' else 'off')

    ...
    def parse_payload(self, payload_dict):
        ...
        for device in device_family:
            for status in device.__status_messages_map.get(payload_dict['message_flag'], []):
                ...
                if m:
                # 첫 번째 캡처 그룹만 꺼내도록 안전하게 처리
                    result[topic] = status['process_func'](m.group(1))
                else:
                    print("Regex match failed:", status['regex'], payload_dict['data'])
        return result
                ...
                try:
                    if self.is_valid(payload_hexstring):
                        ...
                        if m:
                            ...
                            print("Payload regex match failed:", payload_hexstring)
...
### 난방 ###
optional_info = {'hvac_modes': ['off', 'heat'], 'preset_modes': ['none','외출', '온수', '예약'], 'temp_step': 1.0, 'precision': 1.0, 'min_temp': 5.0, 'max_temp': 45.0, 'send_if_off': 'false'}
...
for message_flag in ['81','C3','C5','C7']:
    거실난방.register_status(  message_flag = message_flag, attr_name = 'power', topic_class = 'hvac_mode_state_topic', regex = r'00([\da-fA-F]{2})', process_func = lambda v: 'heat' if format(int(v, 16), '05b')[4] == '1' else 'off')
    안방난방.register_status(  message_flag = message_flag, attr_name = 'power', topic_class = 'hvac_mode_state_topic', regex = r'00([\da-fA-F]{2})', process_func = lambda v: 'heat' if format(int(v, 16), '05b')[3] == '1' else 'off')
    확장난방.register_status(  message_flag = message_flag, attr_name = 'power', topic_class = 'hvac_mode_state_topic', regex = r'00([\da-fA-F]{2})', process_func = lambda v: 'heat' if format(int(v, 16), '05b')[2] == '1' else 'off')
    제인이방난방.register_status(  message_flag = message_flag, attr_name = 'power', topic_class = 'hvac_mode_state_topic', regex = r'00([\da-fA-F]{2})', process_func = lambda v: 'heat' if format(int(v, 16), '05b')[1] == '1' else 'off')
    팬트리난방.register_status(message_flag = message_flag, attr_name = 'power', topic_class = 'hvac_mode_state_topic', regex = r'00([\da-fA-F]{2})', process_func = lambda v: 'heat' if format(int(v, 16), '05b')[0] == '1' else 'off')
...
난방전체.register_command(message_flag = '43', attr_name = 'power', topic_class = 'hvac_mode_command_topic', process_func = lambda v: '01' if v == 'heat' else '00')

거실난방.register_command(message_flag = '43', attr_name = 'power', topic_class = 'hvac_mode_command_topic', process_func = lambda v: '01' if v == 'heat' else '00')
...
안방난방.register_command(message_flag = '43', attr_name = 'power', topic_class = 'hvac_mode_command_topic', process_func = lambda v: '01' if v == 'heat' else '00')
...
확장난방.register_command(message_flag = '43', attr_name = 'power', topic_class = 'hvac_mode_command_topic', process_func = lambda v: '01' if v == 'heat' else '00')
...
제인이방난방.register_command(message_flag = '43', attr_name = 'power', topic_class = 'hvac_mode_command_topic', process_func = lambda v: '01' if v == 'heat' else '00') # { 'ON': '01', 'OFF': '00' }
...
팬트리난방.register_command(message_flag = '43', attr_name = 'power', topic_class = 'hvac_mode_command_topic', process_func = lambda v: '01' if v == 'heat' else '00') # , { 'ON': '01', 'OFF': '00' }
